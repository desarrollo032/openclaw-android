...
const INSTALL_PHASES: { key: string; label: string; skippable?: boolean }[] = [
  ...
  { key: 'pnpm',       label: 'Saltar pnpm (evita uv_cwd)' },
  { key: 'pnpm_env',   label: 'Configurar entorno npm', skippable: true },
  { key: 'versions',   label: 'Verificar versiones', skippable: true },
  { key: 'openclaw',   label: 'Instalar OpenClaw con npm' },
...
export function Setup({ onComplete }: Props) {
        ...
        setPhaseStatus(prev => {
          ...
          switch (e.phaseStatus) {
            ...
            case 'step':
              ...
              state = (existing.state === 'done' || existing.state === 'skipped')
                ? existing.state
                : 'running'
  ...
  }, [refreshStatus, refreshCompletedPhases, appendLog, installing, activePhase])
      ...
      <div className="space-y-3">
        ...
        {(installing || logLines.length > 0) && (
          <div className="rounded-xl bg-black/40 border border-glass-border p-2 max-h-[140px] overflow-y-auto font-mono text-[10px] text-text-muted leading-tight">
            {logLines.length === 0 ? (
              <div className="text-text-dim italic">Esperando salida del instalador…</div>
            ) : (
              logLines.map((line, i) => (
